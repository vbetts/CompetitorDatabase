...
class Competitor(models.Model):
    dataSource = models.ManyToManyField(CompetitorDataSource,
                                        verbose_name="Competitor Data Sources")
    target_Market = models.ManyToManyField(TargetMarket,
                                           verbose_name="Target Markets")
    features = models.ManyToManyField(Feature,
                                      through='CompanyFeatures')
    categories = models.ManyToManyField(Category,
                                        through='CompetitorCategories')
    globalMarket = models.ManyToManyField(GlobalMarket,
                                          through='GlobalMarketShare')
    verticalMarket = models.ManyToManyField(VerticalMarket,
                                            through='VerticalMarketShare')
    ...
    acquired_by = models.CharField(max_length=255,
                                   blank=True)
    merged_with = models.CharField(max_length=255,
                                   blank=True)
...
class CompetitorCategories(models.Model):
    ...
    details = models.TextField("Sub-Categories",
                               blank=True,
                               null=True,
                               max_length=3000)
...
class Product(models.Model):
    ...
    platform = models.ManyToManyField(Platform,
                                      blank=True)
    installation = models.ManyToManyField(Installation,
                                          blank=True)
    urlDB = models.CharField("URL Database",
                             max_length=255,
                             blank=True)
...
class CompanyFeatures(models.Model):
    ...
    featureSpecs = models.TextField("Technical Details",
                                    blank=True,
                                    null=True,
                                    max_length=3000)
...
class Clients(models.Model):
    ...
    industry = models.BooleanField("Industry")
    finance_legal = models.BooleanField("Finance/Legal")
    healthcare = models.BooleanField("Health Care")
    library = models.BooleanField("Libraries")
    security_technology = models.BooleanField("Security/Technology")
    government = models.BooleanField("Government/Police")
    background = models.TextField("Client Background",
                                  max_length=3000,
                                  blank=True)
    use_cases = models.TextField("Use Cases",
                                 max_length=5000,
                                 blank=True)
    netsweeper_uses = models.TextField("Netsweeper Uses",
                                       max_length=5000,
                                       blank=True)
